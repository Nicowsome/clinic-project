...
export default function Settings() {
  ...
  const [userHas2FAEnabled, setUserHas2FAEnabled] = useState(false);

  // Check if user has 2FA enabled on component mount
  useEffect(() => {
    const checkTwoFactorStatus = async () => {
      try {
        ...
        const response = await fetch(`${(import.meta as any).env?.VITE_API_BASE_URL || 'http://localhost:3000/api/v1'}/two-factor/status`, {
          headers: {
        ...
        if (response.ok) {
          ...
          setUserHas2FAEnabled(data.twoFactorEnabled || false);
          setFormData(prev => ({
            ...prev,
            security: {
              ...prev.security,
  ...
  const handleSecurityChange = (key: keyof SettingsFormData['security'], value: any) => {
    // If toggling 2FA, open the dialog instead of just changing the state
            ...
            <List>
              <ListItem>
                <FormControlLabel
                  control={
                    <Switch
                      checked={userHas2FAEnabled}
                      onChange={() => handleSecurityChange('twoFactorAuth', !userHas2FAEnabled)}
                    />
                  }
                  label={`Two-Factor Authentication ${userHas2FAEnabled ? '(Enabled)' : '(Disabled)'}`}
                />
              </ListItem>
              <ListItem>
                <FormControl fullWidth>
      ...
      >
        <DialogTitle>
          <Box display="flex" justifyContent="space-between" alignItems="center">
            ...
            <IconButton edge="end" color="inherit" onClick={handleCloseTwoFactorDialog}>
              <CloseIcon />
            </IconButton>
          </Box>
        </DialogTitle>
        <DialogContent>
          <TwoFactorSettings onComplete={handleTwoFactorSetupComplete} />
        </DialogContent>
      </Dialog>
    </Box>
  );
